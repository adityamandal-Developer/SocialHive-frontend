...
import { fetchingPrivatePostsAc } from "../../reactRedux/slices/posts/postsSlice";
...
const PostLists = () => {
  ...
  //dispatch
  useEffect(() => {
    dispatch(fetchingPrivatePostsAc());
  }, [dispatch]);

  return (
    <>
      <div>
        <section className="relative py-24 bg-blue bg-opacity-95 text-gold">
          <div className="absolute top-0 left-0 w-full h-full" />
          <div className="container relative z-10 px-4 mx-auto">
            <div className="md:max-w-5xl mx-auto mb-8 md:mb-16 text-center">
              <span className="inline-block py-px px-2 mb-4 text-xs leading-5 bg-green-100 font-medium uppercase rounded-full shadow-sm text-gold">
                posts
              </span>
              <h3 className="mb-4 text-3xl md:text-5xl leading-tight font-bold tracking-tighter text-gold">
            ...
            <div className="flex flex-wrap -mx-4 mb-12 md:mb-20">
              {loading ? (
                ...
                posts?.posts?.map((post) => {  //loop  through the posts
                  return (
                    <Link
                      to={`/posts/${post?._id}`}
                    ...
                    >
                      ...
                      <div className="mb-4">
                        <a
                          className="inline-block py-1 px-3 text-xs leading-5 hover:text-green-600 font-medium uppercase bg-green-100 hover:bg-green-200 rounded-full shadow-sm text-gold"
                          href="#"
                        >
                          {post?.category?.name}
                        </a>
                      ...
                      <a
                        className="inline-block mb-4 text-2xl md:text-3xl leading-tight text-coolGray-800 hover:text-coolGray-900 font-bold hover:underline text-gold"
                        href="#"
                      >
                        {post?.title}
                      </a>
                      ...
                      <Link
                        className="inline-flex items-center text-base md:text-lg hover:text-green-600 font-semibold text-gold"
                        to={`/posts/${post?._id}`}
                      >
